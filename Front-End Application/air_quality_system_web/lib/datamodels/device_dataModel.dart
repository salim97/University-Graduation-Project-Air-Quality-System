...
class DeviceDataModel {
  ...
  List<SensorDataModel> getCO() {
    List<SensorDataModel> metricDataModel = new List<SensorDataModel>();

    sensors.forEach((sensor) {
      if (sensor.metric != null) if (sensor.metricName.toLowerCase() == "co") {
        sensor.value = double.parse(sensor.value).toStringAsFixed(1);
        metricDataModel.add(sensor);
      }
    });
    ...
    List<SensorDataModel> getNH3() {
    List<SensorDataModel> metricDataModel = new List<SensorDataModel>();

    sensors.forEach((sensor) {
      if (sensor.metric != null) if (sensor.metricName.toLowerCase() == "nh3") {
        sensor.value = double.parse(sensor.value).toStringAsFixed(1);
        metricDataModel.add(sensor);
      }
    });
    ...
    List<SensorDataModel> getNO2() {
    List<SensorDataModel> metricDataModel = new List<SensorDataModel>();

    sensors.forEach((sensor) {
      if (sensor.metric != null) if (sensor.metricName.toLowerCase() == "no2") {
        sensor.value = double.parse(sensor.value).toStringAsFixed(1);
        metricDataModel.add(sensor);
      }
    });
  ...
  //   List<SensorDataModel> getO3() {
  //   List<SensorDataModel> metricDataModel = new List<SensorDataModel>();

  //   sensors.forEach((sensor) {
  //     if (sensor.metric != null) if (sensor.metricName.toLowerCase() == "o3") {
  //       sensor.value = double.parse(sensor.value).toStringAsFixed(1);
  //       metricDataModel.add(sensor);
  //     }
  //   });
