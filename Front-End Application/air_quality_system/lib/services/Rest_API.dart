...
class RestAPI {
  ...
  Future<WeatherData> getWeather() async {
    http.Response response = await http.get(
      Uri.encodeFull(Endpoints.WEATHER_BY_CITY_ID),
      headers: {
  ...
  Future<ForecastData> getForecast() async {
    http.Response response = await http.get(
      Uri.encodeFull(Endpoints.FORECAST_BY_CITY_ID),
      headers: {
        "Accept": "application/json"
      }
    );
...
const city_id = "35.6987"; // check http://bulk.openweathermap.org/sample/city.list.json.gz
